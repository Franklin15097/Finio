...
import { ArrowDownRight, Plus, Search, SlidersHorizontal } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight">Расходы</h1>
          <p className="text-muted-foreground text-sm mt-0.5">{formatMoney(totalExpense)} за период</p>
        </div>
        <button 
          onClick={() => setShowAdd(true)} 
          className="gradient-expense text-primary-foreground h-11 px-5 rounded-xl font-semibold text-sm flex items-center gap-2 hover:shadow-lg hover:shadow-expense/25 transition-all"
        >
          ...
          {catSummary.map(({ cat, amount }) => (
            <div key={cat?.id} className="glass-card rounded-2xl p-4 min-w-[150px] shrink-0 hover:shadow-md transition-shadow">
              <div className="flex items-center gap-2 mb-2">
                <div className="w-8 h-8 rounded-lg bg-expense/10 text-expense flex items-center justify-center">
              ...
              <p className="text-lg font-extrabold text-expense">{formatMoney(amount)}</p>
            </div>
          ))}
        </div>
      )}
                ...
                {txs.map(tx => {
                  ...
                  return (
                    <div key={tx.id} className="flex items-center gap-4 p-4 group hover:bg-secondary/30 transition-colors first:rounded-t-2xl last:rounded-b-2xl">
                      <div className="w-11 h-11 rounded-xl bg-expense/10 text-expense flex items-center justify-center shrink-0">
                      ...
                      <p className="text-sm font-bold text-expense">−{formatMoney(tx.amount)}</p>
                      <button
      ...
      {showAdd && <AddTransactionModal type="expense" onClose={() => setShowAdd(false)} />}
    </div>
  );
}
